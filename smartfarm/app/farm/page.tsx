...
import { MapPin, Users, Search, Wheat, Droplet, Plus, Beef, HeartPulse, Activity } from "lucide-react";
...
export default function FarmPage() {
  ...
  const fieldStats = useMemo(() => {
    ...
    const avgMoisture = fields.length > 0 
      ? Math.round(fields.reduce((sum, f) => sum + (f.moisture || 0), 0) / fields.length)
      : 0;
      ...
      <div className="space-y-8 pb-10">
        ...
        <header className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500 dark:text-zinc-400 mt-1">Monitor fields, crop cycles, and livestock herds.</p>
          </div>
          
          {activeTab === 'fields' ? (
            <button 
              onClick={() => { setSelectedField(null); setFieldDialogOpen(true); }}
            ...
            <button 
              onClick={() => { setSelectedLivestock(null); setLivestockDialogOpen(true); }}
              className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 px-5 py-2.5 text-white text-sm font-bold shadow-md shadow-emerald-500/20 transition-all cursor-pointer"
            >
              <Plus className="w-4 h-4" />
              Add Livestock
        ...
        <div className="flex border-b border-slate-200 dark:border-zinc-800">
          <button
            onClick={() => { setActiveTab('fields'); setQ(''); }}
          ...
          <button
            onClick={() => { setActiveTab('livestock'); setQ(''); }}
            className={`px-6 py-3 text-sm font-bold flex items-center gap-2 cursor-pointer transition-all border-b-2 -mb-[2px] ${
              activeTab === 'livestock'
                ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400'
                : 'border-transparent text-slate-500 dark:text-zinc-500 hover:text-slate-800 dark:hover:text-zinc-300'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <AnimatePresence mode="wait">
            {activeTab === 'fields' ? (
              <>
                {[
                  { label: "Total Fields", value: fieldStats.count, icon: MapPin, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10" },
                  { label: "Total Area", value: fieldStats.area, icon: Wheat, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/10" },
                  { label: "Workers Active", value: fieldStats.workers, icon: Users, color: "text-sky-600 dark:text-sky-400", bg: "bg-sky-500/10" },
                  { label: "Avg Moisture", value: fieldStats.moisture, icon: Droplet, color: "text-blue-600 dark:text-blue-400", bg: "bg-blue-500/10" },
              ...
              <>
                {[
                  { label: "Total Animals", value: livestockStats.total.toLocaleString(), icon: Beef, color: "text-emerald-600 dark:text-emerald-400", bg: "bg-emerald-500/10" },
                  { label: "Healthy Batches", value: livestockStats.healthy, icon: Activity, color: "text-indigo-600 dark:text-indigo-400", bg: "bg-indigo-500/10" },
                  { label: "Needs Attention", value: livestockStats.attention, icon: HeartPulse, color: "text-rose-600 dark:text-rose-400", bg: "bg-rose-500/10" },
                  { label: "Active Herds", value: livestock.length, icon: Users, color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-500/10" },
                ].map(({ label, value, icon: Icon, color, bg }, i) => (
                  <motion.div
                    key={label}
                    initial={{ opacity: 0, scale: 0.95 }}
              ...
              >
                {filteredFields.length === 0 ? (
                  <div className="col-span-2 flex flex-col items-center justify-center py-20 text-center text-slate-400">
                    ...
                    <h3 className="text-lg font-bold text-slate-600 dark:text-zinc-400">No fields matching your search</h3>
                    <p className="text-sm mt-1 text-slate-500">Log some fields to populate your farm.</p>
                  </div>
                ) : (
                  filteredFields.map((f) => (
                    <FarmCard 
                      key={f.id} 
                      field={f} 
                      onEdit={() => { setSelectedField(f); setFieldDialogOpen(true); }}
              ...
              >
                {filteredLivestock.length === 0 ? (
                  <div className="col-span-3 flex flex-col items-center justify-center py-20 text-center text-slate-400">
                    ...
                    <h3 className="text-lg font-bold text-slate-600 dark:text-zinc-400">No livestock herds matching search</h3>
                    <p className="text-sm mt-1 text-slate-500">Register livestock groups to keep track of counts and checks.</p>
                  </div>
                ) : (
                  filteredLivestock.map((l) => (
                    <LivestockCard 
                      key={l.id} 
                      livestock={l} 
                      onEdit={() => { setSelectedLivestock(l); setLivestockDialogOpen(true); }}
